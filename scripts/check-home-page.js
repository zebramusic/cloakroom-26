...
async function checkHomePage() {
  try {
    ...
    const SitePage = mongoose.model('SitePage', new mongoose.Schema({}, { strict: false, collection: 'sitepages' }));
    ...
    if (homePage) {
      ...
      if (homePage.localeData?.ro?.blocks?.length > 0) {
        ...
        homePage.localeData.ro.blocks.forEach((block, i) => {
          console.log(`${i + 1}. Type: ${block.type}, Visibility: ${block.visibility}`);
          if (block.type === 'hero') {
            console.log('   Background image:', block.data?.backgroundImage || 'None');
