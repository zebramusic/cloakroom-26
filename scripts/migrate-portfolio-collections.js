...
async function migrateCollections() {
  try {
    ...
    const PortfolioItemsSource = mongoose.model(
      "PortfolioItemsSource",
      new mongoose.Schema({}, { strict: false, collection: "portfolioitems" }),
    );
    ...
    const PortfolioItemsTarget = mongoose.model(
      "PortfolioItemsTarget",
      new mongoose.Schema({}, { strict: false, collection: "portfolio_items" }),
    );
    ...
    if (sourceItems.length > 0) {
      ...
      console.log(
        `✓ Copied ${sourceItems.length} items to "portfolio_items"\n`,
      );

      sourceItems.forEach((item) => {
        console.log(
          `  - ${item.localeContent?.ro?.title || "Untitled"} (${item.slug})`,
        );
      });
    }

    // Now check portfolio images collection
    const PortfolioImagesSource = mongoose.model(
      "PortfolioImagesSource",
      new mongoose.Schema({}, { strict: false, collection: "portfolioimages" }),
    );
    const sourceImages = await PortfolioImagesSource.find({}).lean();
    console.log(`\nFound ${sourceImages.length} images in "portfolioimages"`);

    if (sourceImages.length > 0) {
      const PortfolioImagesTarget = mongoose.model(
        "PortfolioImagesTarget",
        new mongoose.Schema(
          {},
          { strict: false, collection: "portfolio_images" },
        ),
      );
      await PortfolioImagesTarget.deleteMany({});
      await PortfolioImagesTarget.insertMany(sourceImages);
      console.log(
        `✓ Copied ${sourceImages.length} images to "portfolio_images"`,
      );
    }
