...
async function checkBothCollections() {
  try {
    ...
    const PortfolioItems = mongoose.model(
      "PortfolioItemsNoUnderscore",
    ...
    const PortfolioItemsUnderscore = mongoose.model(
      "PortfolioItemsWithUnderscore",
    ...
    if (itemsNoUnderscore.length > 0 && itemsWithUnderscore.length === 0) {
      console.log("\n⚠️  ISSUE FOUND:");
      console.log(
        'Items exist in "portfolioitems" but app is looking at "portfolio_items"',
      );
      console.log(
