...
async function testPartnerCRUD() {
  try {
    ...
    // Register model
    Partner = mongoose.models.Partner || mongoose.model('Partner', PartnerSchema);
    ...
    if (existingPartners.length > 0) {
      existingPartners.forEach((p, i) => {
        console.log(`  ${i + 1}. ${p.name} (${p.slug}) - Active: ${p.isActive}`);
      });
    }
    ...
    console.log('─'.repeat(50));
    const activePartners = await Partner.find({ isActive: true }).countDocuments();
    const inactivePartners = await Partner.find({ isActive: false }).countDocuments();
